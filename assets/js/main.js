		...
		this.mainCanvasMult = function(){
			...
			var mainCanvas = function(){
				...
				this.fn.makeEvents = function(){
					that.ev.scroll = $(window).on('scroll', that.fn.render);
				};

				this.fn.calcData = function(){
					...
					$('body').children().each(function(){
						if(this.id !== 'mainCanvas'){
							out.height += $(this).height();
						}
					});
					console.log($(window).height());
					out.scrollMax = out.height - $(window).height();
					out.scroll = $(window).scrollTop();
					out.process = out.scroll * 100 / out.scrollMax;
